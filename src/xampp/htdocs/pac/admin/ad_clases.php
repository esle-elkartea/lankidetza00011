...
$sql="SELECT count(*) FROM ad_clases WHERE 1 $where";
$res=mysql_query($sql);
...
function filaout(elemento){
	...
	<table border=0 width=100% >
		<tr>
			<td align="right"><input type="button" class="Boton" value="  Crear clase  " onClick="window.location='ad_clases_edit.php'"></td>
		</tr>
		<tr>
	...
	$sql="SELECT * FROM ad_clases WHERE 1 $where";
	if($numResultados>0){
		if($res=mysql_query($sql.$where.$order.$limit)){
			...
			while($row=mysql_fetch_array($res)){
				?>
				<tr onMouseOver="filaover(this)"  style="cursor:pointer" onMouseOut="filaout(this)" onClick="window.location='ad_clases_edit.php?id=<?=$row["id_clase"]?>'">
