...
$sql="SELECT count(*) FROM ad_clases_ref WHERE 1 $where";
...
function filaout(elemento){
	...
	<table border=0 width=100% >
		<tr>
			<td align="right"><input type="button" class="Boton" value="  Crear clase  " onClick="window.location='ad_clasesRef_edit.php'"></td>
		</tr>
		<tr>
	...
	$sql="SELECT * FROM ad_clases_ref WHERE 1 $where";
	if($numResultados>0){
		if($res=mysql_query($sql.$where.$order.$limit)){
			...
			while($row=mysql_fetch_array($res)){
				...
				onClick="window.location='ad_clasesRef_edit.php?id=<?=$row["id_clase"]?>'">
			...
			<?
			pintarPaginacion($numResultados,$pag,"clases");
		}
	}else{
		if($where=="") $txt="No hay clases introducidas en la base de datos";
		else $txt="No existen clases con lo parámetros de búsqueda introducidos";		
		?>
