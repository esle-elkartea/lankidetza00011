...
include "../recursos/class/Detectabilidad.class.php";
comprobarAcceso(2,$us_rol);


//***********************************************************************//
if ($_POST["act_guardar"]=="1") {
	$ocu=new Detectabilidad($_GET["id"]);
	$ocu->nombre=txtParaGuardar($_POST["frm_nombre"]);
	$ocu->valor=$_POST["frm_valor"];
	$ocu->controles=txtParaGuardar($_POST["frm_controles"]);
	$ocu->guardar();
}
if ($_POST["act_eliminar"]=="1" && isset($_GET["id"])){
	$ocu=new Detectabilidad($_GET["id"]);
	$ocu->eliminar();
	Header("Location: ad_detecta.php");
}
if ($_POST["act_salir"]=="1") 	Header("Location: ad_detecta.php");
//***********************************************************************//
...
if (!isset($_GET["id"])) $miga_pan.="Nueva Detectabilidad";
...
function validar(f){
	a=new Array();
	a[0]="text::"+f.frm_nombre.value+"::Debe introducir el nombre de la detectabilidad";
	a[1]="int::"+f.frm_valor.value+"::Debe introducir el valor de la detectabilidad::El valor ha de ser un número entero";
	er=JSvFormObligatorios(a);		
	if(er=="") return	true;
	else alert (er);
}
function eliminar (){
	if(confirm("La detectabilidad será eliminada ¿está seguro?")){
...
<table border=0 width=100% >
	<tr>
		<td align="right">
			...
			<input type="button" class="Boton" value="  Salir  "          			onClick="window.location='ad_detecta.php'">
		</td>
	</tr>
...
<table width="100%" border="0" cellspacing="1" cellpadding="0">
	<form method=POST>
		...
		<tr>
	    <td align="center" class="Tit"><span class="fBlanco">DATOS DE LA DETECTABILIDAD</span></td>
	  </tr>
	  <tr>
	    <td align="center" class="Caja">
	        ...
	        <tr>
	          <td width="15%" align="left" class="TxtBold" nowrap valign=top>Controles:</td>
	          <td align="left"><textarea name="frm_controles" type="text" rows=4 cols=70 class="input"><?=txtParaInput($ocu->controles)?></textarea></td>
	        </tr>
	        <tr>
						<td align="left" colspan=2  class="spacer2">&nbsp;</td>
					</tr>
