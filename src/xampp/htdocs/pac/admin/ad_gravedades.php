...
$sql="SELECT count(*) FROM ad_gravedades WHERE 1 $where";
...
function filaout(elemento){
	...
	<table border=0 width=100% >
		<tr>
			<td align="right"><input type="button" class="Boton" value="  Crear gravedad  " onClick="window.location='ad_gravedades_edit.php'"></td>
		</tr>
		<tr>
	...
	$sql="SELECT * FROM ad_gravedades WHERE 1 $where";
	if($numResultados>0){
		if($res=mysql_query($sql.$where.$order.$limit)){
			...
			while($row=mysql_fetch_array($res)){
				?>
				<tr onMouseOver="filaover(this)" onMouseOut="filaout(this)"  style="cursor:pointer"  onClick="window.location='ad_gravedades_edit.php?id=<?=$row["id_gravedad"]?>'">
			...
			<?
			pintarPaginacion($numResultados,$pag,"gravedades");
		}
	}else{
		if($where=="") $txt="No hay gravedades introducidas en la base de datos";
		else $txt="No existen gravedades con lo parámetros de búsqueda introducidos";		
		?>
