...
$sql="SELECT count(*) FROM ad_maquinas WHERE 1 $where";
...
function filaout(elemento){
	...
	<table border=0 width=100% >
		<tr>
			<td align="right"><input type="button" class="Boton" value="  Crear máquina  " onClick="window.location='ad_maquinas_edit.php'"></td>
		</tr>
		<tr>
	...
	$sql="SELECT * FROM ad_maquinas WHERE 1 $where";
	if($numResultados>0){
		if($res=mysql_query($sql.$where.$order.$limit)){
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="1" class="BordesTabla">
		  	<tr>
			    <th width="15%" align="left" nowRAP>&nbsp;Código </th>
			    <th width="85%" align="left" nowRAP>&nbsp;Nombre </th>
			  </tr>	
			<?	
			while($row=mysql_fetch_array($res)){
				?>
				<tr onMouseOver="filaover(this)"  style="cursor:pointer" onMouseOut="filaout(this)" onClick="window.location='ad_maquinas_edit.php?id=<?=$row["id_maquina"]?>'">
			...
			<?
			pintarPaginacion($numResultados,$pag,"maquinas");
		}
	}else{
		if($where=="") $txt="No hay maquinas introducidas en la base de datos";
		else $txt="No existen maquinas con lo parámetros de búsqueda introducidos";		
		?>
