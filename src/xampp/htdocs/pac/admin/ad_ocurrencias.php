...
$sql="SELECT count(*) FROM ad_ocurrencias WHERE 1 $where";
...
function filaout(elemento){
	...
	<table border=0 width=100% >
		<tr>
			<td align="right"><input type="button" class="Boton" value="  Crear ocurrencia  " onClick="window.location='ad_ocurrencias_edit.php'"></td>
		</tr>
		<tr>
	...
	$sql="SELECT * FROM ad_ocurrencias WHERE 1 $where";
	if($numResultados>0){
		if($res=mysql_query($sql.$where.$order.$limit)){
			...
			while($row=mysql_fetch_array($res)){
				?>
				<tr onMouseOver="filaover(this)" style="cursor:pointer"  onMouseOut="filaout(this)" 
				onClick="window.location='ad_ocurrencias_edit.php?id=<?=$row["id_ocurrencia"]?>'">
			...
			<?
			pintarPaginacion($numResultados,$pag,"ocurrencias");
		}
	}else{
		if($where=="") $txt="No hay ocurrencias introducidas en la base de datos";
		else $txt="No existen ocurrencias con lo parámetros de búsqueda introducidos";		
		?>
