...
$sql="SELECT count(*) FROM ad_responsables WHERE 1 $where";
...
function filaout(elemento){
	...
	<table border=0 width=100% >
		<tr>
			<td align="right"><input type="button" class="Boton" value="  Crear Responsable  " onClick="window.location='ad_responsables_edit.php'"></td>
		</tr>
		<tr>
	...
	$sql="SELECT * FROM ad_responsables WHERE 1 $where";
	if($numResultados>0){
		if($res=mysql_query($sql.$where.$order.$limit)){
			...
			<table width="100%" border="0" cellpadding="2" cellspacing="1" class="BordesTabla">
		  	<tr>
			    <th width="15%" align="left" nowRAP>&nbsp;Nombre </th>
			    <th width="85%" align="left" nowRAP>&nbsp;Apellidos </th>
			  </tr>	
			<?	
			while($row=mysql_fetch_array($res)){
				?>
				<tr onMouseOver="filaover(this)"  style="cursor:pointer" onMouseOut="filaout(this)" onClick="window.location='ad_responsables_edit.php?id=<?=$row["id_responsable"]?>'">
		      <td align="left" class="Fila1">&nbsp;<?=$row["nombre"]?>&nbsp;</td>
		      <td align="left" class="Fila1">&nbsp;<?=$row["apellidos"]?>&nbsp;</td>
		    </tr>
				<?			
			}	
			?>
			...
			<?
			pintarPaginacion($numResultados,$pag,"responsables");
		}
	}else{
		if($where=="") $txt="No hay responsables introducidos en la base de datos";
		else $txt="No existen responsables con lo parámetros de búsqueda introducidos";		
		?>
