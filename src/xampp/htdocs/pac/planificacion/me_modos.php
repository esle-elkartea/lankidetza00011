...
if($b_codigo!="" || $b_nombre!="" ){
	$busca=true;
	$where="";
	if($b_codigo!="") $where=" AND codigo LIKE '%".$b_codigo."%'";
	if($b_nombre!="") $where.=" AND nombre LIKE '%".$b_nombre."%'";
...
<!--LISTADO-->
	...
	$sql = 	"SELECT id_modo,nombre,codigo FROM modos ".
					"WHERE 1 $where";
	if($numResultados>0){
		if($res=mysql_query($sql.$where.$order.$limit)){
			...
			while($row=mysql_fetch_array($res)){
				?>
				<tr onMouseOver="filaover(this)" onMouseOut="filaout(this)" onClick="window.location='me_modos_edit.php?id=<?=$row["id_modo"]?>'">
		...
		else $txt="No hay modos con lo parámetros de búsqueda introducidos";
		?>
